...
class PdfHelper {
	...
	*/
	...
	public function __construct($settings = array()) {
		...
		$this->pdf->setSourceFile($assetDir.$this->template);
	...
	*/
	public function add() {
		...
		$this->pdf->AddPage();
		if($this->letterHead) {
			// $this->pdf->Image($assetDir.'letter_bg.png', 0, 65, 53, 200);
			// $this->pdf->Image($assetDir.'letter_footer.png', 77, 268, 50, 5);

			if($this->pdf->getPage() == 1) {
				$this->pdf->useTemplate($this->pdf->importPage(1));
				// $this->pdf->Image($assetDir.'letter_ais.png', 150, 10, 50, 66);
			}
			else {
				$this->pdf->useTemplate($this->pdf->importPage(2));
				// $this->pdf->Image($assetDir.'letter_ais_pg2.png', 150, 10, 50, 66);
			}
		}
